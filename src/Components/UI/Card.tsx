...
const Card = ({ details, type, theme }: Props) => {
  ...
  if (type === 'course') {
    ...
    const url = `/Article/${details._id}/${encodeURIComponent(details?.urlGoogle ?? details?.title).replace(/%20/g, '-')}`;
    return (
    ...
    const buyHandler = useCallback(
      (item: 'course' | 'book') => {
        addToBasket(Auth, dispatch, details, item);
      },
      [details],
    );
    if ('yearOfPublication' in details) {
      const url = `/Book/${details._id}/${encodeURIComponent(details.title).replace(/ /g, '-')}}`;
      return (
